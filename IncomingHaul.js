...
(() => {
  ...
  // Loading...
  $('div#show_outgoing_units').prepend(
    $(
      '<div style="height:30px;"><strong style="font-size: 20px;">TOTAL: </strong><span id="haul-message">Processando...</span></div>',
    ),
  );
  ...
  const incCmdTable = document.querySelector(
    '.commands-container table:nth-child(1)',
  ); // Incoming commands table
          ...
          function (response) {
            ...
            if (responseHaul) {
              Object.keys(haulData).forEach((key) => {
                ...
                elm.getElementsByClassName('haul-item-' + key)[0].innerText =
                  haulData[key];
              });
            }
