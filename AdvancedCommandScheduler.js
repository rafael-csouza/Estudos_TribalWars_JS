...
(async (ModuleLoader) => {
  ...
  const CommandSender = {
    ...
    init: function () {
      ...
      this.duration = $('#command-data-form')
        ...
        .map(Number);
      this.internetDelay =
        localStorage.getItem('ACS.internetDelay') || defaultInternetDelay;
      ...
      $('#ACSbutton').click(function () {
        ...
        setTimeout(function () {
          ...
          ((day, hour, minute, second, millisecond) => {
            ...
            var _nextFn = () => {
              const realOffset =
                parseInt(CommandSender.internetDelay) - worldBackwardDelay;
              console.log('ACS.realOffset = ', realOffset);
              if (
                CommandSender.createServerDate(realOffset).getSeconds() >=
                second
              ) {
                console.log(
                  'ACS.second = ',
              ...
              worker.onmessage = function () {
                if (_is_Done) {
                  UI.Notification.show(
                    'https://dsbr.innogamescdn.com/asset/c092731a/graphic/unit/recruit/axe.png',
                    'Parabéns!',
                    'Seu comando foi enviado com sucesso!',
                  );
  ...
  CommandSender.addGlobalStyle(
    '#ACStime, #ACSInternetDelay {font-size: 9pt;font-family: Verdana,Arial;}#ACSbutton {float:right;}',
  );
