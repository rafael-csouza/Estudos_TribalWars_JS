...
(() => {
  ...
  const _solveCaptchaFunction = (_documentReference) => {
    ...
    if (_documentReference.location) {
      if (
        _documentReference.location.href.indexOf('google.com/recaptcha') > -1
      ) {
        var clickCheck = setInterval(function () {
          if (
            _documentReference.querySelectorAll('.recaptcha-checkbox-checkmark')
              .length > 0
          ) {
            clearInterval(clickCheck);
            _documentReference
              .querySelector('.recaptcha-checkbox-checkmark')
              .click();
          }
        }, 100);
      }
